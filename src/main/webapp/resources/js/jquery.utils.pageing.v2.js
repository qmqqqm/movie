		...
		return this.each(function(i, v) {
			...
			*/
			function addItemToContainer(target) {
				var strDay = '';
				target['spanDay'] === '' ? strDay = '' : strDay = '<span> . </span><em>' + target['spanDay'] + '</em>';
				...
				var url = target['url'], 
					...
					$returnValue = $('<li><a id="tile_' + index + '" href="'+target['link']+'"><div class="evt-thumb"></div><div class="evt-desc"><p class="txt1"></p><p class="txt2">' + eventTerm  + spanDay + '</p></div></a></li>'),
                ...
                $('<img>').attr({'src': url, 'alt' : alt}).on('error', function (e) {errorImage(this,{type:'square'});}).appendTo($thumb);
